// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'share.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
  'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#adding-getters-and-methods-to-our-models',
);

ShareEntity _$ShareEntityFromJson(Map<String, dynamic> json) {
  return _ShareEntity.fromJson(json);
}

/// @nodoc
mixin _$ShareEntity {
  String get shareUrl => throw _privateConstructorUsedError;
  String get token => throw _privateConstructorUsedError;
  String get type => throw _privateConstructorUsedError;

  /// Serializes this ShareEntity to a JSON map.
  Map<String, dynamic> toJson() => throw _privateConstructorUsedError;

  /// Create a copy of ShareEntity
  /// with the given fields replaced by the non-null parameter values.
  @JsonKey(includeFromJson: false, includeToJson: false)
  $ShareEntityCopyWith<ShareEntity> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $ShareEntityCopyWith<$Res> {
  factory $ShareEntityCopyWith(
    ShareEntity value,
    $Res Function(ShareEntity) then,
  ) = _$ShareEntityCopyWithImpl<$Res, ShareEntity>;
  @useResult
  $Res call({String shareUrl, String token, String type});
}

/// @nodoc
class _$ShareEntityCopyWithImpl<$Res, $Val extends ShareEntity>
    implements $ShareEntityCopyWith<$Res> {
  _$ShareEntityCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  /// Create a copy of ShareEntity
  /// with the given fields replaced by the non-null parameter values.
  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? shareUrl = null,
    Object? token = null,
    Object? type = null,
  }) {
    return _then(
      _value.copyWith(
            shareUrl: null == shareUrl
                ? _value.shareUrl
                : shareUrl // ignore: cast_nullable_to_non_nullable
                      as String,
            token: null == token
                ? _value.token
                : token // ignore: cast_nullable_to_non_nullable
                      as String,
            type: null == type
                ? _value.type
                : type // ignore: cast_nullable_to_non_nullable
                      as String,
          )
          as $Val,
    );
  }
}

/// @nodoc
abstract class _$$ShareEntityImplCopyWith<$Res>
    implements $ShareEntityCopyWith<$Res> {
  factory _$$ShareEntityImplCopyWith(
    _$ShareEntityImpl value,
    $Res Function(_$ShareEntityImpl) then,
  ) = __$$ShareEntityImplCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call({String shareUrl, String token, String type});
}

/// @nodoc
class __$$ShareEntityImplCopyWithImpl<$Res>
    extends _$ShareEntityCopyWithImpl<$Res, _$ShareEntityImpl>
    implements _$$ShareEntityImplCopyWith<$Res> {
  __$$ShareEntityImplCopyWithImpl(
    _$ShareEntityImpl _value,
    $Res Function(_$ShareEntityImpl) _then,
  ) : super(_value, _then);

  /// Create a copy of ShareEntity
  /// with the given fields replaced by the non-null parameter values.
  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? shareUrl = null,
    Object? token = null,
    Object? type = null,
  }) {
    return _then(
      _$ShareEntityImpl(
        shareUrl: null == shareUrl
            ? _value.shareUrl
            : shareUrl // ignore: cast_nullable_to_non_nullable
                  as String,
        token: null == token
            ? _value.token
            : token // ignore: cast_nullable_to_non_nullable
                  as String,
        type: null == type
            ? _value.type
            : type // ignore: cast_nullable_to_non_nullable
                  as String,
      ),
    );
  }
}

/// @nodoc
@JsonSerializable()
class _$ShareEntityImpl implements _ShareEntity {
  const _$ShareEntityImpl({
    this.shareUrl = '',
    this.token = '',
    this.type = '',
  });

  factory _$ShareEntityImpl.fromJson(Map<String, dynamic> json) =>
      _$$ShareEntityImplFromJson(json);

  @override
  @JsonKey()
  final String shareUrl;
  @override
  @JsonKey()
  final String token;
  @override
  @JsonKey()
  final String type;

  @override
  String toString() {
    return 'ShareEntity(shareUrl: $shareUrl, token: $token, type: $type)';
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$ShareEntityImpl &&
            (identical(other.shareUrl, shareUrl) ||
                other.shareUrl == shareUrl) &&
            (identical(other.token, token) || other.token == token) &&
            (identical(other.type, type) || other.type == type));
  }

  @JsonKey(includeFromJson: false, includeToJson: false)
  @override
  int get hashCode => Object.hash(runtimeType, shareUrl, token, type);

  /// Create a copy of ShareEntity
  /// with the given fields replaced by the non-null parameter values.
  @JsonKey(includeFromJson: false, includeToJson: false)
  @override
  @pragma('vm:prefer-inline')
  _$$ShareEntityImplCopyWith<_$ShareEntityImpl> get copyWith =>
      __$$ShareEntityImplCopyWithImpl<_$ShareEntityImpl>(this, _$identity);

  @override
  Map<String, dynamic> toJson() {
    return _$$ShareEntityImplToJson(this);
  }
}

abstract class _ShareEntity implements ShareEntity {
  const factory _ShareEntity({
    final String shareUrl,
    final String token,
    final String type,
  }) = _$ShareEntityImpl;

  factory _ShareEntity.fromJson(Map<String, dynamic> json) =
      _$ShareEntityImpl.fromJson;

  @override
  String get shareUrl;
  @override
  String get token;
  @override
  String get type;

  /// Create a copy of ShareEntity
  /// with the given fields replaced by the non-null parameter values.
  @override
  @JsonKey(includeFromJson: false, includeToJson: false)
  _$$ShareEntityImplCopyWith<_$ShareEntityImpl> get copyWith =>
      throw _privateConstructorUsedError;
}
