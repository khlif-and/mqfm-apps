...
class _VotePageState extends State<VotePage>
              ...
              child: BlocConsumer<VoteBloc, VoteState>(
                ...
                builder: (context, state) {
                  return state.maybeWhen(
                    ...
                    rankingLoaded: (rankings) => _buildRankingList(rankings),
  ...
  Widget _buildRankingList(List<VoteRankingEntity> rankings) {
    if (rankings.isEmpty) {
      return Center(
        child: Padding(
          padding: EdgeInsets.symmetric(horizontal: AppDims.w24),
        ...
        itemBuilder: (context, index) {
          ...
          return RepaintBoundary(
            child: GestureDetector(
              onTap: () => context.push(
                AppPathRoutes.playerWithId(ranking.audioId.toString()),
              ),
              child: VoteRankingTile(
                ranking: ranking,
                onVote: () {
                  _voteBloc.add(VoteEvent.cast(audioId: ranking.audioId));
                },
              ),
            ),
          );
